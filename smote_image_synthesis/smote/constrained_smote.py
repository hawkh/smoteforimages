...
class ConstrainedSMOTE:
    ...
    def _filter_by_distance(self, 
        ...
        for label in unique_labels:
            # Indices of synthetic samples for this label
            syn_indices = np.where(synthetic_labels == label)[0]
            if len(syn_indices) == 0:
                continue

            # Original embeddings for this label
            ...
            # Compute distances from all synthetic samples of this class to all original samples of this class
            # We only need the min distance for each synthetic sample
