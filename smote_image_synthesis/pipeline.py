    ...
    def _compute_repulsion_loss(
        ...
        for lbl in unique_labels:
            ...
            if class_embs.size(0) < 2:
                continue
            # Pairwise distances
            # Optimization: Use torch.cdist instead of unsqueeze dimension expansion
            # to prevent O(K^2 * D) memory allocation and improve performance.
            dists = torch.cdist(class_embs.float(), class_embs.float(), p=2.0)  # [K, K]
