...
class QualityAssessor:
    ...
    def compute_diversity_metrics(self, synthetic_images: torch.Tensor) -> Dict[str, float]:
        ...
        # Ensure float32 or float64 for cdist
        if flattened.dtype != torch.float32 and flattened.dtype != torch.float64:
            flattened = flattened.float()

        # Compute pairwise distances efficiently on device
        ...
        if len(upper_triangle) > 0:
            ...
            # PyTorch std is unbiased by default, numpy is biased by default
