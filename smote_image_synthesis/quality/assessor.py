...
class QualityAssessor:
    ...
    def compute_diversity_metrics(self, synthetic_images: torch.Tensor) -> Dict[str, float]:
        ...
        # Use torch.cdist to avoid CPU/NumPy transfers and utilize GPU if available
        ...
        # Using device=distances.device prevents CPU-GPU synchronization overhead
        upper_triangle_indices = torch.triu_indices(
            batch_size, batch_size, offset=1, device=distances.device
        )
        upper_triangle = distances[upper_triangle_indices[0], upper_triangle_indices[1]]

        if len(upper_triangle) > 0:
            results['mean_pairwise_distance'] = float(torch.mean(upper_triangle))
            # unbiased=False matches numpy's default standard deviation
            std_dist = torch.std(upper_triangle, unbiased=False)
            results['std_pairwise_distance'] = float(std_dist)
            ...
            # Theoretical max
            max_possible_dist = float(np.sqrt(np.prod(images.shape[1:])))
            if max_possible_dist > 0:
                results['diversity_index'] = mean_dist / max_possible_dist
            else:
                results['diversity_index'] = 0.0
        else:
            results = {
