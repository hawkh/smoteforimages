...
class QualityAssessor:
    ...
    def compute_diversity_metrics(self, synthetic_images: torch.Tensor) -> Dict[str, float]:
        ...
        # Compute pairwise distances using PyTorch to avoid CPU transfers
        # and utilize GPU acceleration if available
        distances = torch.cdist(flattened, flattened, p=2)
        
        # Extract upper triangle (excluding diagonal) natively in PyTorch
        ...
        if len(upper_triangle) > 0:
            ...
            # unbiased=False matches the default behavior of numpy.std
