...
class QualityAssessor:
    ...
    def compute_diversity_metrics(self, synthetic_images: torch.Tensor) -> Dict[str, float]:
        ...
        # Compute pairwise distances efficiently using PyTorch native functions
        # This avoids expensive CPU/GPU transfers and leverages PyTorch optimizations
        distances = torch.cdist(flattened, flattened, p=2)
        
        # Extract upper triangle (excluding diagonal)
        triu_idx = torch.triu_indices(batch_size, batch_size, offset=1, device=flattened.device)
        upper_triangle = distances[triu_idx[0], triu_idx[1]]
        
        if len(upper_triangle) > 0:
            results['mean_pairwise_distance'] = float(torch.mean(upper_triangle))
            results['std_pairwise_distance'] = float(torch.std(upper_triangle))
