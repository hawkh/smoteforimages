...
class QualityAssessor:
    ...
    def compute_diversity_metrics(self, synthetic_images: torch.Tensor) -> Dict[str, float]:
        ...
        # Compute pairwise distances efficiently using PyTorch to avoid CPU transfers
        ...
        row_idx, col_idx = torch.triu_indices(
            batch_size, batch_size, offset=1, device=images.device
        )
        upper_triangle = distances[row_idx, col_idx]
        
        if len(upper_triangle) > 0:
            results['mean_pairwise_distance'] = float(torch.mean(upper_triangle))
            results['std_pairwise_distance'] = float(
                torch.std(upper_triangle, unbiased=False)
            )
